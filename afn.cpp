...
unordered_map<char, vector<int>> M[301];
...
bool Evaluare(char s[])
{
    ...
    for(i = 0; i < lungime; i++)
    {
        ...
        while(Q1.empty() == 0)  ///punem in Q2 starile in care putem ajunge cu lambda
        {
            x = Q1.front();
            Q1.pop();
            Q2.push(x);
        ...
        while(Q2.empty() == 0)  ///punem in Q1 starile in care putem ajunge cu caracterul curent
        {
            x = Q2.front();
            Q2.pop();
            for(int y : M[x][s[i]])
    ...
    while(Q1.empty() == 0)  ///chiar daca s-a terminat cuvantul, 
    {                       ///inca mai putem parcurge graful cu labda
        x = Q1.front();
        Q1.pop();
        Q2.push(x);
...
///parcurgerea grafului folosind backtracking
void Parcurgere_lambda(int q, int iq, int l)
{
    drum[iq] = q;
    viz[q] = iq;
    ...
    for(int x : M[q][s[l]])
    {
        if(viz[x] != iq + 1)
        {
            ...
            if(SolutieFinala(x, l))
                ...
                Parcurgere_lambda(x, iq + 1, l);
        }
    }
    
    if(ok == 0) Parcurgere_lambda(q, iq, l);
}
